...
public class LuaUsageFilteringRuleProvider implements UsageFilteringRuleProvider {
    ...
    static class LuaOverrideFilteringRule implements UsageFilteringRule {

        @Override
        public boolean isVisible(@NotNull Usage usage, @NotNull UsageTarget @NotNull [] targets) {
            if (usage instanceof ReadWriteAccessUsageInfo2UsageAdapter adapter) {
                return !adapter.isAccessedForReading() || !adapter.isAccessedForWriting();
